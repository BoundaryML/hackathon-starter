...
async def get_session_state(request: Request) -> State:
    ...
    session_id = "123"  # only 1 session (global state for now)
    return session_store.get_state(session_id)
...
async def query(
    ...
    message: str,
    timestamp: int,
    state: State = Depends(get_session_state),
):
    ...
    state.recent_messages.append(
        Message(role=Role.User, content=message, timestamp=timestamp)
    )
            ...
            async for chunk in res:
                ...
                try:
                    ...
                    if current_command.type == "get_weather_report":
                        ...
                        weather_report = await tool_handlers.get_weather(
                            current_command.location
                        )
                    ...
                    if current_command.type == "compute_value":
                        ...
                        value = await tool_handlers.compute_value(
                            current_command.arithmetic_expression
                        )
                        logger.info(f"Value: {value}")
                        state.recent_messages.append(
                            Message(
                                role=Role.Assistant,
                                content=f"{current_command.arithmetic_expression} = {value}",
                                timestamp=unix_now(),
                            )
                        )
                        processed_commands.add(command_index)
                    ...
                    if current_command.type == "message_to_user":
                        ...
                        content = (
                            ""
                            if message_to_user.message.value is None
                            else message_to_user.message.value
                        )
